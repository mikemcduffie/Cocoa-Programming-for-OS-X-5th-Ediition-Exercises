...
class Rocket: Particle {
    ...
    let altitudeToDeploy: Double = 400.0
    let parachuteLift = Vector(x: 0, y: 9.2)
    ...
    override func tick(dt: NSTimeInterval) {
        if thrustTimeRemaining > 0.0 {
            ...
            if self.position.y < altitudeToDeploy {
                acceleration = acceleration + parachuteLift
            }
        }
        super.tick(dt)
    }
...
class Simulation {
    ...
    func tick(dt: NSTimeInterval) {
        ...
        particles = particles.filter { particle in
            let live = particle.position.y > 0.0 // TRUE if postion y is  greater than zero
            if !live {
                print("Particle terminated at time \(self.time). Terminal velocity = \(particle.velocity.y)m/s.")
            }
            return live
        }
    }
